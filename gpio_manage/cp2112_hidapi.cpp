...
int CP2112_HIDAPI::cp2112_gpio_get(unsigned gpio)
{
    hidStatus = deviceCheck();
    if (hidStatus < 0)
    {
...
int CP2112_HIDAPI::gpio_high(uint8 GPIO)
    {
        //Set GPIO open/high
        int status = -1;
        status = set_gpio( 1 << GPIO , 1 << GPIO);
        return status;
    }

int CP2112_HIDAPI::gpio_low(uint8 GPIO)
    {
        //Set GPIO low
        int status = -1;
        status = set_gpio(1 << GPIO , 0x00);
        return status;
    }
