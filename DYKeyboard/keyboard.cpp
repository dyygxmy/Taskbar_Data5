...
// insert and show text on display(QLineEdit) 中英文切换
...
{
//    qDebug()<<"setHanziText:"<<text;
	if (text.isNull() || text.isEmpty())
		return ;
	int length = text.length();
	for (int i = 0; i < length; i++)
        emit characterGenerated(text.mid(i,1));
